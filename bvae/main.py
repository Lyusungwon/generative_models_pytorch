...
def train(epoch):
    ...
    for batch_idx, input_data in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            writer.add_scalar('Batch  Reconstruction Loss', recon_loss / batch_size, epoch)
            writer.add_scalar('Batch  KL Divergence Loss', kld_loss.sum() / batch_size, epoch)
            summary = {}
            kl_divergence = kld_loss.sum(0).view(-1)
            for i in range(kl_divergence.size()[0]):
                summary['Batch KL Divergence loss (detail) {}'.format(i)] = kl_divergence[i] / batch_size
